...
use super::size::{PoolSize, DecreaseOnDrop, IncreaseGuard};
    ...
    C: Connection + Connect<Connection = C>,
{
    pub(super) async fn new_arc(url: &str, options: Options) -> crate::Result<Arc<Self>> {
        ...
        for guard in (0..pool.options.min_size).filter_map(|_| pool.size.try_increase()) {
            let live = pool
                .eventually_connect(Instant::now() + pool.options.connect_timeout, guard)
                .await?;
    ...
    pub(super) async fn close(&self) {
        ...
        while let Some(_) = self.waiters.pop() {}
    ...
    #[inline]
    pub(super) fn try_acquire(&self) -> Option<Floating<C>> {
    ...
    pub(super) async fn acquire(&self) -> crate::Result<Floating<C>> {
        ...
        while !self.is_closed.load(Ordering::Acquire) {
            ...
            if let Some(idle) = self.idle.pop().ok() {
                let guard = self.size.decrease_on_drop();
                if let Some(live) = check_live(idle.live, &self.options, guard).await {
                    return Ok(live);
                }
            }
            ...
            let live = match timeout(until, rx).await {
                ...
                // waiter was dropped for some reason
                Ok(Err(_)) => continue,
            ...
            if self.is_closed.load(Ordering::Acquire) {
                // `live` is detached from the pool
                live.close().await;
                return Err(Error::PoolClosed);
            }

            match check_live(live, &self.options, guard).await {
    ...
    async fn eventually_connect(&self, deadline: Instant, guard: IncreaseGuard) -> crate::Result<Live<C>> {
        loop {
    ...
    async fn connect(&self, deadline: Instant, guard: IncreaseGuard) -> crate::Result<Option<Live<C>>> {
...
async fn check_live<C>(mut live: Live<C>, options: &Options, guard: DecreaseOnDrop) -> Option<Live<C>>
where
    C: Connection,
{
